...
const HomepageContent = () => {
  ...
  const loadMoreContent = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://ec2-54-211-127-150.compute-1.amazonaws.com:3000/api/content/homepage?page=${page}`, {
        headers: {
  ...
  const handleLike = async (contentId) => {
    ...
    await axios.put(`http://ec2-54-211-127-150.compute-1.amazonaws.com:3000/api/content/like/${contentId}`, {}, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`
      }
    });
  ...
  const handleSave = async (contentId) => {
    await axios.put(`http://ec2-54-211-127-150.compute-1.amazonaws.com:3000/api/content/save/${contentId}`, {}, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`
      }
    });
