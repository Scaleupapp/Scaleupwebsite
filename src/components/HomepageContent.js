...
const HomepageContent = () => {
  ...
  const loadMoreContent = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`https://ec2-54-211-127-150.compute-1.amazonaws.com/api/content/homepage?page=${page}`, {
        headers: {
  ...
  const handleLike = async (contentId) => {
    ...
    await axios.put(`https://ec2-54-211-127-150.compute-1.amazonaws.com/api/content/like/${contentId}`, {}, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`
      }
    });
  ...
  const handleSave = async (contentId) => {
    await axios.put(`https://ec2-54-211-127-150.compute-1.amazonaws.com/api/content/save/${contentId}`, {}, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`
      }
    });
